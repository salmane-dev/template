...
const Navbar = ({logo,sitename}) => {

  const [openNav, setOpenNav] = useState(false);
  const wrapperRef = useRef(null);
  useOutsideAlerter(wrapperRef, setOpenNav);

  const menu_item = "menu_item text-lg font-semibold text-blue-800 transition-all "
      ...
      <a href="/" className="pl-7 lg:ml-10">
        <h2 className="text-2xl font-bold" >{sitename}</h2>
             ...
             openNav &&
              <div ref={wrapperRef} className="absolute top-0 w-full h-48 bg-white">
                ...
                <div className="flex flex-col items-center justify-around h-full pt-5">
                  <a onClick={()=>setOpenNav(false)} href="/" className={menu_item}>Home</a>
                  <a onClick={()=>setOpenNav(false)} href="/privacy" className={menu_item}>Privacy</a>
                  <a onClick={()=>setOpenNav(false)} href="/Unsubscribe" className={menu_item}>Unsubscribe</a>
                  <a onClick={()=>setOpenNav(false)} href="/#contact" className={menu_item}>Contact</a>
                </div>
              </div>
          }
          ...
          <div className="flex justify-around w-2/5 mr-12 lg:visible sm:invisible">
              ...
              <a href="/privacy" className={menu_item}>Privacy</a>
              <a href="/Unsubscribe" className={menu_item}>Unsubscribe</a>
